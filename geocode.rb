...
providers_without_geo.each do |p|
  ...
  geo_results = JSON.parse(RestClient.get("http://www.datasciencetoolkit.org/maps/api/geocode/json?sensor=false&address="+URI.encode(address_to_lookup.gsub(/[^a-zA-Z\d\s,]/,""))))
